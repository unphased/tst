...
export const spawn_must_show_full_error_on_failure = test('spawnAsync', async ({ l, spawn, a: { eq, eqO, match } }) => {
  const projDir = getBuildProjDir();
  l('projDir:', projDir);
  const ret = await spawn('node', ['--enable-source-maps', path.join(projDir, 'dispatch', 'runner.js'), TestLaunchFlags.ExactTestNameMatching, TestLaunchFlags.ForceEnableLogging, path.join(projDir, 'tests/self.js'), TestLaunchSeparator, 'spawnAsync:spawn_a_failing_command'], {
